...
function multiplyArrayByN(arr, cb) { //its is a higher order function
  let finalArr = [];
  for (let elm of arr) {
...
function addFive(n) { //its is a callback function.
  return n + 5;
}
function multiplyBy5(n) {  //its is a callback function.
  return n * 5;
}
